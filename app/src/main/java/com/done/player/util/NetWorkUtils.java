...
public class NetWorkUtils {
    ...
    private static String RET_CODE = "code";
    private static String RET_MSG = "msg";
    ...
    public static void postUrl1(final Activity mContext, String url, Map<String, String> params,
                                final IListener listener) {
        postUrl1(mContext, url, params, true, listener);
    }
...
//							}
    ...
    public static void postUrl1(final Activity mContext, String url, Map<String, String> params, final boolean isLock,
                                final IListener listener) {
        if (!NetWorkManager.isNetworkAvailable(mContext)) {
            CommonToastUtils.showInCenterToast(mContext, "您当前网络不可用");
            listener.onError("您当前网络不可用", "", "");
        ...
        OkHttpUtils.post(url, params, new OkHttpUtils.ICallback() {
            ...
            public void onSuccess(final String result) {
                mContext.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            LogUtils.i("44444-------", result);
                            String result1 = result.replaceAll("\n", "");
                            JSONObject resObj = new JSONObject(result1);
                            String code = resObj.optString(RET_CODE);
                            if (SUC_FLAG.equals(code)) {
                                listener.onSuccess(result1, resObj);
                                LogUtils.i("44444", "1111");
                            } else {
                                listener.onSuccess(result1, resObj);
                                LogUtils.i("33333", "222222");
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        } finally {
